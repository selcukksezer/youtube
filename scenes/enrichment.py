...
def enforce_visual_cadence_14(scenes: List[Dict[str, Any]], min_cadence: int = 14) -> List[Dict[str, Any]]:
    ...
    Maintains exact total duration while guaranteeing that no two scenes have duplicate narration.
    """
    if not scenes:
        return scenes
    ...
    for sc in scenes:
        ...
        if cuts_needed > 0 and curr_dur >= 3.5:
            ...
            sc2["narration"] = f"{orig_narration} [Görsel Açı {cutaway_counter}]"
            cutaway_counter += 1
    ...
    while len(expanded) < min_cadence:
        ...
        t2["narration"] = f"{orig_narr} [Kadraj {cutaway_counter}]"
        cutaway_counter += 1
