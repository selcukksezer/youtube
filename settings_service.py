...
def get_dashboard_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "google_ai": {
            ...
            "use_gemini_image_gen": getattr(config, "USE_GEMINI_IMAGE_GEN", False),
...
def _dashboard_env_updates() -> Dict[str, str]:
    return {
        ...
        "USE_GEMINI_IMAGE_GEN": str(getattr(config, "USE_GEMINI_IMAGE_GEN", False)).lower(),
        ...
        "TTS_RATE": config.TTS_RATE or "+18%",
        "TTS_PITCH": getattr(config, "TTS_PITCH", "+0Hz") or "+0Hz",
        "ENABLE_BGM": str(config.ENABLE_BGM).lower(),
        "BGM_VOLUME": str(config.BGM_VOLUME),
        "DEFAULT_BGM_TRACK": getattr(config, "DEFAULT_BGM_TRACK", "") or "",
        "SUBTITLE_COLOR": str(getattr(config, "SUBTITLE_COLOR", "white")),
        "SUBTITLE_HIGHLIGHT_COLOR": str(getattr(config, "SUBTITLE_HIGHLIGHT_COLOR", "#FFD700")),
        "SUBTITLE_FONT_SIZE": str(getattr(config, "SUBTITLE_FONT_SIZE", 54)),
        "SUBTITLE_Y_POSITION": str(getattr(config, "SUBTITLE_Y_POSITION", 0.8)),
