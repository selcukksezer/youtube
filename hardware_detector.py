            ...
            if res.returncode == 0:
                for line in (res.stdout or "").splitlines():
                    if "CPU die temperature" in line or "CPU Temperature" in line:
                        parts = line.split(":")
                        if len(parts) > 1:
                            num = "".join(c for c in parts[1] if c.isdigit() or c == ".")
                            if num:
                                temp_c = float(num)
...
def get_gpu_info() -> Dict[str, Any]:
    ...
    # Check FFmpeg hardware encoder support (NVENC on Windows NVIDIA, VideoToolbox on macOS)
    try:
        import imageio_ffmpeg
        ffmpeg_exe = imageio_ffmpeg.get_ffmpeg_exe()
        res_enc = subprocess.run([ffmpeg_exe, "-encoders"], capture_output=True, text=True, timeout=4)
        enc_out = res_enc.stdout or ""
        if "h264_nvenc" in enc_out and has_nvidia:
            has_nvenc = True
        if platform.system() == "Darwin" and "h264_videotoolbox" in enc_out:
            has_videotoolbox = True
            if not has_nvidia:
                gpu_name = "Apple VideoToolbox (Donanım Hızlandırma)"
    except Exception:
        pass
...
def get_system_hardware_specs() -> Dict[str, Any]:
    ...
    if gpu.get("has_videotoolbox"):
        ...
        profile_label = f"macOS VideoToolbox Donanim Hizlandirma ({cpu['threads'] // 2} Thread CPU)"
        ...
        recommended_threads = min(8, max(4, cpu["threads"] // 2))
        use_gpu = True
    elif thermal.get("thermal_throttle_recommended"):
        recommended_profile = "balanced"
        profile_label = "Termal koruma — dusuk thread (Item 450)"
        recommended_threads = max(2, min(4, cpu["threads"] // 3))
        use_gpu = False
