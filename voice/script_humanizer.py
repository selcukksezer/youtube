...
def sanitize_ai_cliches(text: str) -> str:
    ...
    cliches = [
        ...
        (r'\b(abone\s+olmayı\s+ve\s+beğenmeyi\s+unutmayın|abone\s+olmayı\s+unutmayın)[!.,;:]*\s*', ''),
        (r'^\s*(bu\s+videoda|bu\s+shorts\'?ta)[,:]?\s*', ''),
        (r'\b(işte\s+detaylar)[!.,;:]*\s*', 'bütün gerçekler ortaya çıktı.'),
        (r'\b(işte\s+bilmeniz\s+gerekenler)[!.,;:]*\s*', 'tüm gerçekler burada.'),
        (r'\b(yorumlar(da)?\s+(buluşalım|yazın|yaz|paylaşın|paylaş|tartışalım))\b[.!?,;:]?\s*', ''),
        (r'^\s*(hello\s+(everyone|guys)|welcome\s+to\s+my\s+channel)[!.,;:]*\s*', ''),
        (r'\b(don\'t\s+forget\s+to\s+(like\s+and\s+)?subscribe|like\s+and\s+subscribe|comment\s+below)\b[.!?,;:]*\s*', ''),
